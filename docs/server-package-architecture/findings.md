...
- Round 2 Phase 15 env wiring connected durable request logging to the app through `REQUEST_LOGGING_ENABLED`. The default remains disabled, and enabled deployments log only sanitized route, method, status, duration, request id, user id, and source via `Logger.logRequest()`.
