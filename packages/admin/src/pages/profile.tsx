...
import {
  useUpdateUserProfileCustomData,
  useUserProfileCustomData,
  useUserProfileSchema,
} from '../api/user-profiles'
...
export function ProfilePage() {
  ...
  const profileSchema = useUserProfileSchema()
  const customProfile = useUserProfileCustomData(data?.id)
  const updateCustomProfile = useUpdateUserProfileCustomData(data?.id)
  ...
  async function handleCustomProfileSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    ...
    for (const field of profileSchema.data?.fields || []) {
      if (field.hidden) continue
      const raw = form.get(field.name)
      if (field.type === 'boolean' || field.type === 'checkbox') {
        customData[field.name] = raw === 'on'
      } else if (field.type === 'number') {
        customData[field.name] = raw === null || raw === '' ? null : Number(raw)
      } else {
        customData[field.name] = raw === null ? '' : String(raw)
      }
    }

    await updateCustomProfile.mutateAsync({ customData })
    setCustomSaved(true)
  }
  ...
  return (
    <section className="space-y-10">
      ...
      {profileSchema.data && profileSchema.data.fields.length > 0 && (
        <form onSubmit={handleCustomProfileSubmit} className="space-y-4 max-w-lg">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Custom profile</h2>

          {customProfile.isError && (
            <Alert title="Custom profile failed to load" tone="danger">Try refreshing.</Alert>
          )}
          {updateCustomProfile.isError && (
            <Alert title="Save failed" tone="danger">
              {updateCustomProfile.error instanceof AdminApiError ? updateCustomProfile.error.message : 'Unexpected error'}
            </Alert>
          )}
          {customSaved && <Alert title="Saved" tone="success">Custom profile updated.</Alert>}

          {profileSchema.data.fields.filter(field => !field.hidden).map((field) => {
            const value = customProfile.data?.customData[field.name] ?? field.default ?? ''
            const commonClass = 'grid gap-1.5'

            if (field.type === 'select') {
              return (
                <div key={field.name} className={commonClass}>
          ...
          <Button type="submit" disabled={updateCustomProfile.isPending || !data?.id}>
            {updateCustomProfile.isPending ? 'Saving…' : 'Save custom profile'}
          </Button>
        </form>
      )}
    </section>
