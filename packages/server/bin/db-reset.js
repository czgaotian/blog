...
async function main() {
  log('\n🔧 Worker Blog Database Reset Tool', colors.cyan + colors.bold);
  log('================================\n', colors.cyan);
  ...
  if (!wranglerInfo) {
    ...
    log('Please run this command from your Worker Blog project directory.', colors.yellow);
    process.exit(1);
  }
  ...
  const dbName = `worker-blog-worktree-${safeBranch}`;
