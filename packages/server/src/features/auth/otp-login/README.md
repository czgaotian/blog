...
Passwordless authentication for Worker Blog via email one-time codes (OTP). Users sign in by entering a 6-digit code sent to their email - no password required.
...
- ✅ **Email Integration**: Uses Worker Blog email plugin
...
The JWT issued by `/auth/otp/verify` uses the core Worker Blog session config:
