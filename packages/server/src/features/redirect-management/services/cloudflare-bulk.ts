...
const LIST_NAME = 'worker-blog-redirects'
const RULESET_NAME = 'Worker Blog Bulk Redirects'
...
/**
 * Get or create the redirect list named "worker-blog-redirects"
 */
export async function getOrCreateList(config: CloudflareConfig): Promise<{ success: boolean; listId?: string; error?: string }> {
  ...
  const createResponse = await cfFetch<CloudflareList>(config, listPath, {
    ...
    body: JSON.stringify({
      name: LIST_NAME,
      kind: 'redirect',
      description: 'Worker Blog managed redirects - auto-synced from admin panel'
    })
  })
...
export async function getOrCreateRule(config: CloudflareConfig, _listId: string): Promise<{ success: boolean; ruleId?: string; error?: string }> {
  ...
  const createResponse = await cfFetch<CloudflareRuleset>(config, rulesetPath, {
    ...
    body: JSON.stringify({
      ...
      description: 'Worker Blog bulk redirects - auto-synced from admin panel',
      rules: [{
        action: 'redirect',
        expression: `http.request.full_uri in $${LIST_NAME}`,
        description: 'Redirect from Worker Blog managed list',
        action_parameters: {
          from_list: {
            name: LIST_NAME,
...
/**
 * Convert a Worker Blog redirect to Cloudflare list item format
 */
