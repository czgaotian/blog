...
**GitHub Issue**: [#362 - Advanced search with Cloudflare Search](https://github.com/lane711/worker-blog/issues/362)
...
Unlike traditional approaches that require manual dashboard configuration, this plugin implements a **Custom RAG (Retrieval-Augmented Generation)** pipeline that works automatically for any Worker Blog user.
...
```bash
cd packages/server
bash ../packages/server/src/plugins/core-plugins/ai-search-plugin/setup/vectorize-setup.sh
```
...
```typescript
import { AISearchService } from '@worker-blog/server/plugins'
...
1. **Check Vectorize setup**:
   ...
   # Should show: worker-blog-search
   ```
...
- [Worker Blog Search](./README.md)

### Issues:

- [GitHub Issues](https://github.com/lane711/worker-blog/issues)
- [Issue #362](https://github.com/lane711/worker-blog/issues/362)
...
- Works for ANY Worker Blog user
