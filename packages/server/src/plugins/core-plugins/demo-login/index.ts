...
import type { Plugin, PluginContext, HookHandler } from '@worker-blog/shared/types/plugin'

/**
 * Demo Login Plugin
 *
 * Prefills the login form with demo credentials (admin@worker-blog.com/worker-blog!)
    ...
    (function() {
      ...
      function prefillLoginForm() {
        ...
        if (emailInput && passwordInput) {
          emailInput.value = 'admin@worker-blog.com';
          passwordInput.value = 'worker-blog!';
...
const demoLoginPlugin = PluginBuilder.create({
  ...
  .metadata({
    description: 'Prefills login form with demo credentials (admin@worker-blog.com/worker-blog!) for easy site demonstration',
    author: {
      name: 'Worker Blog'
    },
    dependencies: []
