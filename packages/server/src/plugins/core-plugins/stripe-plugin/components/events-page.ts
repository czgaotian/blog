...
import { renderAdminLayoutCatalyst, type AdminLayoutCatalystData } from '@worker-blog/admin/templates/layouts/admin-layout-catalyst.template'
...
function extractUserInfo(event: StripeEventRecord): { userId?: string; email?: string } {
  try {
    ...
    const userId = data?.metadata?.worker_blog_user_id || data?.worker_blog_user_id
