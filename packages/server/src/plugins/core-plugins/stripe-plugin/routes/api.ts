  ...
  try {
    switch (event.type) {
      case 'customer.subscription.created': {
        ...
        const userId = sub.metadata?.worker_blog_user_id || await subscriptionService.getUserIdByStripeCustomer(sub.customer) || ''
      ...
      case 'checkout.session.completed': {
        ...
        const userId = session.metadata?.worker_blog_user_id
  ...
  if (!customerId) {
    ...
    if (existing) {
      ...
      const customer = await stripeApi.createCustomer({
        email: user.email,
        metadata: { worker_blog_user_id: user.userId }
      })
