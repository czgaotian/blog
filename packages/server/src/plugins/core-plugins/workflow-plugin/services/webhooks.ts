  ...
  private async deliverWebhook(
    ...
    try {
      ...
      const headers: Record<string, string> = {
        ...
        'User-Agent': 'Worker Blog-Webhooks/1.0',
        'X-Worker Blog-Event': eventType,
        'X-Worker Blog-Delivery': deliveryId,
        'X-Worker Blog-Timestamp': timestamp
      }

      // Add signature if secret is provided
      if (webhook.secret) {
        const signature = await this.createSignature(JSON.stringify(webhookPayload), webhook.secret)
        headers['X-Worker Blog-Signature'] = signature
      }
