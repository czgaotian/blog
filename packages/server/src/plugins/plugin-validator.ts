...
import { z } from 'zod'
import { Plugin, PluginValidator as IPluginValidator, PluginValidationResult, PluginRegistry } from '@worker-blog/shared/types'
...
export class PluginValidator implements IPluginValidator {
  ...
  /**
   * Validate plugin compatibility with Worker Blog version
   */
  validateCompatibility(plugin: Plugin, sonicVersion: string): PluginValidationResult {
    ...
    try {
      if (!semver.satisfies(sonicVersion, plugin.compatibility)) {
        errors.push(`Plugin requires Worker Blog ${plugin.compatibility}, but current version is ${sonicVersion}`)
      }
    } catch (error) {
