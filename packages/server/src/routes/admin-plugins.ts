...
import { renderPluginsListPage, PluginsListPageData, Plugin } from '@worker-blog/admin/templates/pages/admin-plugins-list.template'
import { renderPluginSettingsPage, PluginSettingsPageData } from '@worker-blog/admin/templates/pages/admin-plugin-settings.template'
...
adminPluginRoutes.get('/:id', async (c) => {
  try {
    ...
    if (pluginId === 'otp-login') {
      ...
      const siteName = generalSettings.siteName || 'Worker Blog'
